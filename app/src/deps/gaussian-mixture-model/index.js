...
import * as ct from 'cholesky-tools';
...
export class GMM {
	...
	runEM(iterations = 1, itCallback = (it) => { }) {
		if(this.dataLength==0) return;
		for(let i=0; i<iterations; i++) {
			itCallback(i);
			this.runExpectation.call(this);
			this.runMaximization.call(this);
// calculate Cholesky decompositions of covariances
	...
	runExpectation() {
		...
		for(let k=0; k<this.clusters; k++) {
			...
			console.log(mean, cov, covDet, covCholesky);
	...
	pdf(x, mean, cov, covDet, covCholesky) {  // probability density function
		...
		let mah2 = this.xmuAxmu(ct.inverse(cov, L), mean, x);  // mahalanobis^2
